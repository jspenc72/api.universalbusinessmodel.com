...
$v8 = "'" . $conn -> real_escape_string($activeTaskId) . "'";
 ...
 $sql="DELETE FROM ubm_model_has_policies WHERE policy_id=$v5 AND model_id=$v2";
 if($conn->query($sql) === false) {
 	trigger_error('Wrong SQL: ' . $sql . ' Error: ' . $conn->error, E_USER_ERROR);
 } else {
 	$affected_rows = $conn->affected_rows;	
		 $sql="DELETE FROM ubm_model_jobdescription_has_policies WHERE jobdescription_id=$v4 AND policy_id=$v5";
		 if($conn->query($sql) === false) {
		 	trigger_error('Wrong SQL: ' . $sql . ' Error: ' . $conn->error, E_USER_ERROR);
		 } else {
		 	$affected_rows = $conn->affected_rows;
