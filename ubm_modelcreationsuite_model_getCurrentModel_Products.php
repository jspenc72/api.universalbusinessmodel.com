...
//1. Select all records for checklist items stored in model_creation_suite, Count the number of items in the checklist.
			$sqlsel1="SELECT * FROM ubm_model_has_products WHERE model_UUID=$activeModelId";		//Select all 
			$rs1=$conn->query($sqlsel1);
			if($rs1 === false) {
			  trigger_error('Wrong SQL: ' . $sqlsel1 . ' Error: ' . $conn->error, E_USER_ERROR);
			} else{
				while ($items1 = $rs1->fetch_assoc()) {
					$returnProductsId = stripslashes($items1['product_id']);
					$sqlsel2="SELECT * FROM ubm_model_products WHERE id=$returnProductsId";		//Select all 
