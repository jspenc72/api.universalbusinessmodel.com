...
$v8 = "'" . $conn -> real_escape_string($activeTaskId) . "'";
 ...
 $sql="DELETE FROM ubm_model_has_procedures WHERE procedure_id=$v6 AND model_id=$v2";
 if($conn->query($sql) === false) {
 	trigger_error('Wrong SQL: ' . $sql . ' Error: ' . $conn->error, E_USER_ERROR);
 } else {
 	$affected_rows = $conn->affected_rows;	
	 $sql="DELETE FROM ubm_model_policy_has_procedures WHERE procedure_id=$v6 AND policy_id=$v5";
	 if($conn->query($sql) === false) {
	 	trigger_error('Wrong SQL: ' . $sql . ' Error: ' . $conn->error, E_USER_ERROR);
	 } else {
	 	$affected_rows = $conn->affected_rows;
