...
$v8 = "'" . $conn -> real_escape_string($activeTaskId) . "'";
 ...
 $sql="DELETE FROM ubm_model_has_steps WHERE step_id=$v7 AND model_id=$v2";
 if($conn->query($sql) === false) {
 	trigger_error('Wrong SQL: ' . $sql . ' Error: ' . $conn->error, E_USER_ERROR);
 } else {
 	$affected_rows = $conn->affected_rows;	
	 $sql="DELETE FROM ubm_model_procedure_has_steps WHERE step_id=$v7 AND procedure_id=$v6";
	 if($conn->query($sql) === false) {
	 	trigger_error('Wrong SQL: ' . $sql . ' Error: ' . $conn->error, E_USER_ERROR);
	 } else {
	 	$affected_rows = $conn->affected_rows;
